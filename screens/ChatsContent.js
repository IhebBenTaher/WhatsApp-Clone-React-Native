        ...
        keyExtractor={(item) => item.id}
         renderItem={({ item }) => 
          <View style={styles.chatItemContainer}>
            <Text style={styles.chatItem}>{item.email}</Text>
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            {/* Search field in the modal */}
            <TextInput
              style={styles.searchInput}
              placeholder="Search Users (email, fullname, phone, pseudo)"
              value={modalSearchTerm}
              onChangeText={setModalSearchTerm} // Update search term for modal
            />

            <FlatList
              data={filteredUsers} // Use filtered users based on search term
              keyExtractor={(item) => item.id} // Use user ID as key
              renderItem={({ item }) => (
                <TouchableOpacity onPress={() => handleAddChat(item.id)}>
                  <Text style={styles.userEmail}>{item.email} - {item.fullname}</Text>
                </TouchableOpacity>
              )}
            />
            <Button title="Close" onPress={() => setModalVisible(false)} color="#e74c3c" />
          </View>
        </View>
      </Modal>
    </View>
